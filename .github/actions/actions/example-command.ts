...
const assertExpectedEvent = pipe(
  ...
  Effect.catchTag('UnexpectedEvent', ({ eventName }) =>
...
export default Command.make(
  ...
  Effect.fnUntraced(function* (inputs) {
    ...
    yield* ui.group(
      ...
      Effect.gen(function* () {
        yield* ui.notice('This is a notice annotation - informational message', {
          ...
          file: '.github/actions/actions/example/action.ts',
          startLine: 73,
          endLine: 75,
        });
        yield* ui.warning('This is a warning annotation - something to be aware of', {
    ...
    yield* ui.debug('This debug message is only visible when debug logging is enabled');
