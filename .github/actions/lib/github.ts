...
const getToken = Effect.gen(function* () {
  ...
  // Fallback to gh CLI (uses $secret to avoid logging the token)
  const ghToken = yield* $secret`gh auth token`.pipe(
...
/**
 * Get repository from GITHUB_REPOSITORY env var, falling back to git remote origin
 */
const getRepository = Effect.gen(function* () {
  const envRepo = yield* Config.option(GitHubConfig.repository);

  if (Option.isSome(envRepo)) {
    return envRepo.value;
  }

  // Fallback to git remote origin
  const remoteUrl = yield* $`git remote get-url origin`.pipe(
    Effect.mapError((cause) =>
      ConfigError.MissingData(
        [
          'GITHUB_REPOSITORY',
        ],
        `GITHUB_REPOSITORY not set and 'git remote get-url origin' failed: ${cause.cause.message}`,
      ),
    ),
  );
  ...
  if (!match) {
    return yield* Effect.fail(
      ConfigError.MissingData(
        ...
        `Could not parse owner/repo from git remote: ${remoteUrl}`,
      ),
    );
  }
...
const makeLive = Effect.gen(function* () {
  ...
  // ConfigProvider with token for effect-octokit-layer
  const tokenConfigProvider = ConfigProvider.fromMap(
    new Map([
      [
        'GITHUB_TOKEN',
        Redacted.value(tokenRedacted),
      ],
    ]),
  );

  // OctokitLayerLive with token config provided
  const OctokitLayerWithToken = OctokitLayerLive.pipe(
    Layer.provide(Layer.setConfigProvider(tokenConfigProvider)),
  );
  ...
  const getPR = (prNumber: number): Effect.Effect<PullRequest, GitHubError> =>
    OctokitLayer.repo(repo)
      ...
      .pipe(
        ...
        Effect.provide(OctokitLayerWithToken),
      );
...
const makeMock = Effect.gen(function* () {
  ...
  // Mock uses hardcoded default since it doesn't make real API calls
  const repository = 'local/mock-repo';
  const repo = parseRepo(repository);
