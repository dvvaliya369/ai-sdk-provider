 ...
 * @category errors
 */
export class ActionEventPayloadError extends Data.TaggedError(
  'ActionEventPayloadError',
)<{
...
const make = (payload: Option.Option<Record<string, unknown>>) => {
  ...
  return { payload, get, issueNumber, prNumber } as const;
};
...
export class ActionEventPayload extends Effect.Service<ActionEventPayload>()(
  ...
  static readonly Mock = (payload: Record<string, unknown>) =>
    Layer.succeed(
      ActionEventPayload,
      new ActionEventPayload(make(Option.some(payload))),
    );
