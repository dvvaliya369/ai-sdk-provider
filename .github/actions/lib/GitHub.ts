...
export type TaggedEventPayload<E extends WebhookEventName> = {
  readonly [K in E]: { readonly _tag: K } & EventPayloadMap[K];
}[E];
...
export class GitHubError extends Data.TaggedError('GitHubError')<{
  ...
  static make = (operation: string, message: string, cause?: unknown) =>
    new GitHubError({ operation, message, cause });
}
...
export const GitHubConfig = {
  eventName: Config.string('GITHUB_EVENT_NAME').pipe(
    Config.withDefault('local'),
  ),

  repository: Config.string('GITHUB_REPOSITORY').pipe(
    Config.withDefault('local/mock-repo'),
  ),
...
export interface GitHubService {
  ...
  readonly whenEvent: <E extends Array<WebhookEventName>>(
    ...
    TaggedEventPayload<E[number]>,
    UnexpectedEvent | GitHubError
  >;
 ...
 * @category services
 */
export class GitHub extends Context.Tag('@openrouter/GitHub')<
  GitHub,
  GitHubService
>() {}
...
const makeLive = Effect.gen(function* () {
  ...
  > =>
    Effect.gen(function* () {
      ...
      if (Option.isNone(eventPayload.payload)) {
        return yield* Effect.fail(
          GitHubError.make('whenEvent', 'No event payload available'),
        );
      }
  ...
  const getCurrentPR: Effect.Effect<PullRequest, GitHubError> = Effect.gen(
    function* () {
      const prNumber = eventPayload.prNumber;

      if (Option.isNone(prNumber)) {
        return yield* Effect.fail(
          GitHubError.make('getCurrentPR', 'Not in a pull request context'),
        );
      }
  ...
  const getDiff = (pr: PullRequest): Effect.Effect<string, GitHubError> =>
    Effect.gen(function* () {
      if (!rawOctokit) {
        return yield* Effect.fail(
          GitHubError.make('getDiff', 'GITHUB_TOKEN not available'),
        );
      }
      ...
      const response = yield* Effect.tryPromise({
        try: () =>
          ...
          GitHubError.make(
            'getDiff',
            `Failed to get diff for PR #${pr.number}`,
            cause,
          ),
      });
  ...
  const comment = (
    ...
    Effect.gen(function* () {
      if (!rawOctokit) {
        return yield* Effect.fail(
          GitHubError.make('comment', 'GITHUB_TOKEN not available'),
        );
      }
      ...
      yield* Effect.tryPromise({
        try: () =>
          ...
          GitHubError.make(
            'comment',
            `Failed to post comment on #${issueNumber}`,
            cause,
          ),
      });
    });
...
const makeMock = Effect.gen(function* () {
  const eventName = yield* Effect.orDie(GitHubConfig.eventName);
...
`),
    ...
    comment: (prNumber: number, body: string) =>
      Effect.gen(function* () {
        yield* Effect.log(
          `[Mock GitHub] Would post comment to PR #${prNumber}:`,
        );
        yield* Effect.log(
